...
/*   Updated: 2024/09/25 21:56:00 by seonseo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	key_hook_rotate_object(int keycode, t_vars *vars)
{
	...
	if (obj->type == SPHERE)
		...
		vector = &obj->data.plane.normal;
	else if (obj->type == CYLINDER)
		vector = &obj->data.cylinder.side.axis;
	else
		vector = &obj->data.cone.side.axis;
...
void	key_hook_translate_object(int keycode, t_vars *vars)
{
	...
	obj = vars->select.curr_obj;
	if (obj->type == SPHERE)
		pos = &obj->data.sphere.center;
	else if (obj->type == PLANE)
		pos = &obj->data.plane.pos;
	else if (obj->type == CYLINDER)
		pos = &obj->data.cylinder.side.center;
	else
		pos = &obj->data.cone.side.vertex;
