...
/*   Updated: 2024/09/24 14:30:51 by damin            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	set_amb_light(char **line, t_light *light)
{
	if (ft_strslen(line) != 3)
		return \
		(error_return("Error\nAmbient light format: A [intensity] [color]", \
		PERROR_OFF));
...
int	set_point_light(char **line, t_light *light)
{
	if (ft_strslen(line) != 4)
		return \
		(error_return("Error\nPoint light format: L [pos] [intensity] [color]", \
		PERROR_OFF));
	...
	if (light->intens < 0 || light->intens > 1)
		return \
		(error_return("Error\nPoint light intensity must be between 0 and 1", \
		PERROR_OFF));
