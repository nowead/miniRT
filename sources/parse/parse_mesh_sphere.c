...
/*   Updated: 2024/09/10 21:47:42 by seonseo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	spheres = (t_sphere *)ft_calloc(1, sizeof(t_sphere));
	if (!spheres)
		return (1);
...
int		set_sphere(char **line, t_sphere *spheres)
{
	...
	if (check_coord(coord))
		return (1);
	spheres->center = (t_point3){ft_atof(coord[0]), ft_atof(coord[1]), ft_atof(coord[2])};
	free_lists(coord);
...
int	parse_spheres(char **line,t_vars *vars)
{
	...
	else
	{
		...
		while (curr->next)
			curr = curr->next;
		curr->next = (t_sphere *)ft_calloc(1, sizeof(t_sphere));
