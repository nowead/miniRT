...
/*   Updated: 2024/09/18 19:36:09 by seonseo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	intersect_ray_cone_side(t_inter_vars *vars)
{
	...
	{
		if (is_p_within_cone_height(term[VO_DOT_AXIS], term[D_DOT_AXIS], t[0], side->height))
			update_closest_hit(t[0], SIDE, vars);
		if (t[0] != t[1] && is_p_within_cone_height(term[VO_DOT_AXIS], term[D_DOT_AXIS], t[1], side->height))
...
void	intersect_ray_cone_cap(t_inter_vars *vars)
{
	...
	if (compute_circle_intersection(vars->ray, &vars->obj->data.cone.bottom_cap, &t))
