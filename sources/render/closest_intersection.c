...
/*   Updated: 2024/09/24 14:39:05 by seonseo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_scene *scene)
{
	...
	{
		if (obj->type == SPHERE)
			intersect_ray_sphere((t_inter_vars){&ray, obj, &t_range, \
			&closest_hit});
		else if (obj->type == PLANE)
			intersect_ray_plane((t_inter_vars){&ray, obj, &t_range, \
			&closest_hit});
		else if (obj->type == CYLINDER)
			intersect_ray_cylinder((t_inter_vars){&ray, obj, &t_range, \
			&closest_hit});
		else if (obj->type == CONE)
			intersect_ray_cone((t_inter_vars){&ray, obj, &t_range, \
			&closest_hit});
		obj = obj->next;
	}
	return (closest_hit);
