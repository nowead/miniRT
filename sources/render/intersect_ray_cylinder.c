...
/*   Updated: 2024/09/24 16:53:01 by seonseo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	intersect_ray_cylinder_side(t_inter_vars *vars)
{
	...
	{
		if (is_p_within_cylinder_height(term[CO_DOT_AXIS], term[D_DOT_AXIS], \
		t[0], cylinder->height))
...
void	compute_cylinder_side_quadratic_coefficients(t_inter_vars *vars, \
float coeff[3], t_vec3 *co, float term[2])
{
	t_cy_side	*cylinder;
	t_vec3		d_perp;
	t_vec3		co_perp;
	...
	cylinder = &vars->obj->data.cylinder.side;
	d_perp = subtract_vectors(vars->ray->dir, \
	scale_vector(cylinder->axis, term[D_DOT_AXIS]));
	co_perp = subtract_vectors(*co, \
	scale_vector(cylinder->axis, term[CO_DOT_AXIS]));
